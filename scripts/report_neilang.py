"""
Generates a self-contained markdown report (images embedded as base64)
from neilang prediction results.
Usage: python3 report_neilang.py [csv_file]
If no file is given, the latest neilang_predictions_*.csv in exports/ is used.
"""
...
def load_latest_csv() -> Path:
    candidates = sorted(EXPORTS_DIR.glob("neilang_predictions_*.csv"))
    if not candidates:
        raise FileNotFoundError("No neilang_predictions_*.csv found in exports/")
    return candidates[-1]
...
def load_data(csv_path: Path) -> pd.DataFrame:
    ...
    for col in ["gt_neilang", "pred_neilang", "match", "age"]:
...
def compute_metrics(df: pd.DataFrame) -> dict:
    valid  = df.dropna(subset=["gt_neilang", "pred_neilang", "match"])
    labels = sorted(set(valid["gt_neilang"].astype(int)) | set(valid["pred_neilang"].astype(int)))
    gt     = valid["gt_neilang"].astype(int).tolist()
    pred   = valid["pred_neilang"].astype(int).tolist()
    ...
    return {
        ...
        "parse_fail": int(df["pred_neilang"].isna().sum()),
...
def plot_pred_vs_gt_heatmap(metrics: dict) -> str:
    """Normalised row-wise heatmap: for each GT class, what does the model predict?"""
    labels = metrics["labels"]
    cm     = metrics["cm"]
...
def build_report(df: pd.DataFrame, metrics: dict, csv_path: Path, ts: str) -> str:
    ...
    p_norm   = plot_pred_vs_gt_heatmap(metrics)
    print("  distribution...")
    ...
    region_acc = (df.dropna(subset=["match", "region"])
                  .groupby("region")["match"].agg(["mean", "count"]).reset_index()
                  .sort_values("mean", ascending=False))
    region_rows = "\n".join(
        f"| {r['region']} | {int(r['count'])} | {r['mean']:.4f} |"
        for _, r in region_acc.iterrows()
    )

    gender_acc = (df.dropna(subset=["match", "gender"])
                  .groupby("gender")["match"].agg(["mean", "count"]).reset_index()
                  .sort_values("mean", ascending=False))
    gender_rows = "\n".join(
        f"| {r['gender']} | {int(r['count'])} | {r['mean']:.4f} |"
        for _, r in gender_acc.iterrows()
    )
    ...
    return f"""# neilang Prediction Report
...
- The model shows a strong bias toward class **2 (A little bothered)** — predicted {m['pred_dist'].get(2,0)} times vs GT {m['gt_dist'].get(2,0)}.
- Class **3 (Very bothered)** is the hardest: recall = **{pc[3]['recall']:.4f}** (support = {pc[3]['support']}).
...
def main():
    ...
    PLOTS_DIR = FIGURES_DIR / f"neilang_plots_{ts_file}"
    PLOTS_DIR.mkdir(parents=True, exist_ok=True)

    print("Generating plots...")
    report = build_report(df, metrics, csv_path, ts)

    report_path = REPORTS_DIR / f"neilang_report_{ts_file}.md"

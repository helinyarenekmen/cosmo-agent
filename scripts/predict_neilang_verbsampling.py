...
- Çıktı: exports/neilang_verbsampling_<timestamp>.csv + .json
...
# Verbalized sampling: modelden doğrudan olasılık dağılımı istenir.
# Olasılıklar toplamı 1.0 olmalı ve normalize edilir.
QUESTION = (
    ...
    '{"p1": <0-1 arası>, "p2": <0-1 arası>, "p3": <0-1 arası>}\n\n'
    "Soru:\n"
...
def parse_response(text: str) -> tuple[dict[str, float] | None, int | None, int | None]:
    """
    Returns (probs_dict, sampled_pred, argmax_pred).
    probs_dict: {"p1": float, "p2": float, "p3": float} (normalized, sum=1)
    sampled_pred: label drawn from the distribution (1/2/3)
    argmax_pred:  label with highest probability (1/2/3)
    """
    if not text:
    ...
    if obj is None:
        # Fallback: regex extract p1/p2/p3
        m1 = re.search(r'"p1"\s*:\s*([0-9]*\.?[0-9]+)', text)
        m2 = re.search(r'"p2"\s*:\s*([0-9]*\.?[0-9]+)', text)
        m3 = re.search(r'"p3"\s*:\s*([0-9]*\.?[0-9]+)', text)
        if m1 and m2 and m3:
            obj = {"p1": float(m1.group(1)), "p2": float(m2.group(1)), "p3": float(m3.group(1))}
        else:
            return None, None, None

    try:
        p1 = float(obj.get("p1", 0))
        p2 = float(obj.get("p2", 0))
        p3 = float(obj.get("p3", 0))
    ...
    probs = {"p1": round(p1, 6), "p2": round(p2, 6), "p3": round(p3, 6)}

    # Argmax
    argmax_pred = max([(p1, 1), (p2, 2), (p3, 3)])[1]
...
def call_api(client: OpenAI, system_prompt: str) -> tuple[dict | None, int | None, int | None, str, int, int]:
...
def save_results(results: list[dict], ts: str):
    csv_path  = EXPORTS_DIR / f"neilang_verbsampling_{ts}.csv"
    json_path = EXPORTS_DIR / f"neilang_verbsampling_{ts}.json"
...
def print_summary(results: list[dict]):
    ...
    # Ortalama olasılık dağılımı özeti
    valid_probs = [r for r in results if r["p1"] is not None]
    if valid_probs:
        avg_p1 = sum(r["p1"] for r in valid_probs) / len(valid_probs)
        avg_p2 = sum(r["p2"] for r in valid_probs) / len(valid_probs)
        avg_p3 = sum(r["p3"] for r in valid_probs) / len(valid_probs)
        print(f"\n── Ortalama Olasılık Dağılımı ({'n='+str(len(valid_probs))}) ──────────────────")
        print(f"  p(Hiç=1)   : {avg_p1:.4f}")
        print(f"  p(Biraz=2) : {avg_p2:.4f}")
        print(f"  p(Çok=3)   : {avg_p3:.4f}")
...
if __name__ == "__main__":
    ...
    checkpoint_path = EXPORTS_DIR / "neilang_verbsampling_checkpoint.json"

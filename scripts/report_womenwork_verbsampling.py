"""
Generates a markdown report from womenwork verbalized-sampling prediction results.
Usage: python3 report_womenwork_verbsampling.py [csv_file]
If no file is given, the latest womenwork_verbsampling_*.csv in exports/ is used.
"""
...
LABEL_MAP = {
    1: "Strongly disagree (1)", 2: "Disagree (2)", 3: "Neither (3)",
...
def load_latest_csv() -> Path:
    candidates = sorted(EXPORTS_DIR.glob("womenwork_verbsampling_*.csv"))
    if not candidates:
        raise FileNotFoundError("No womenwork_verbsampling_*.csv found in exports/")
    return candidates[-1]
...
def plot_mean_probs_by_gt(df: pd.DataFrame) -> str:
    valid  = df.dropna(subset=["gt_womenwork", "p1", "p2", "p3", "p4", "p5"])
    labels = sorted(valid["gt_womenwork"].astype(int).unique())
    p_cols = ["p1", "p2", "p3", "p4", "p5"]
    means  = valid.groupby("gt_womenwork")[p_cols].mean()

    x, w = np.arange(len(p_cols)), 0.15
    palette = ["#4C72B0", "#DD8452", "#55A868", "#8172B2", "#C44E52"]
    fig, ax = plt.subplots(figsize=(9, 4))
    for i, lbl in enumerate(labels):
        vals = [means.loc[lbl, c] if lbl in means.index else 0 for c in p_cols]
        ax.bar(x + (i - 2) * w, vals, w,
               label=f"GT: {LABEL_SHORT[lbl]}", color=palette[i % 5], alpha=0.85)
    ax.set_xticks(x)
    ax.set_xticklabels([f"p({LABEL_SHORT[j+1]})" for j in range(5)], fontsize=9)
...
def plot_entropy(df: pd.DataFrame) -> str:
    valid = df.dropna(subset=["gt_womenwork", "p1", "p2", "p3", "p4", "p5"]).copy()
    eps   = 1e-9
    valid["entropy"] = -sum(
        valid[f"p{i}"] * np.log2(valid[f"p{i}"] + eps) for i in range(1, 6)
    )
    labels  = sorted(valid["gt_womenwork"].astype(int).unique())
    palette = ["#4C72B0", "#DD8452", "#55A868", "#8172B2", "#C44E52"]
    ...
    for i, lbl in enumerate(labels):
        subset = valid[valid["gt_womenwork"] == lbl]["entropy"]
        ax.hist(subset, bins=20, alpha=0.45,
                label=LABEL_SHORT[lbl], color=palette[i % 5], edgecolor="white")
...
def build_report(df: pd.DataFrame, metrics: dict, csv_path: Path, ts: str) -> str:
    ...
    vp  = df.dropna(subset=["p1", "p2", "p3", "p4", "p5"])
    eps = 1e-9
    ent = -sum(vp[f"p{i}"] * np.log2(vp[f"p{i}"] + eps) for i in range(1, 6))
    avg_probs = {f"p{i}": vp[f"p{i}"].mean() for i in range(1, 6)}
    high_conf = (ent < 0.5).sum()

    return f"""# womenwork Prediction Report — Verbalized Sampling
...
|---|---|---|---|---|---|
| Overall mean | {avg_probs['p1']:.4f} | {avg_probs['p2']:.4f} | {avg_probs['p3']:.4f} | {avg_probs['p4']:.4f} | {avg_probs['p5']:.4f} |
...
def main():
    ...
    PLOTS_DIR = FIGURES_DIR / f"womenwork_verbsampling_plots_{ts_file}"
